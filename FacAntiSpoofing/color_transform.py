...
def deal(rgb):
    y = RGB2YCbCr(rgb)
    # b = (y[:, :, 1] >= 80) & (y[:, :, 1] <= 130) & (y[:, :, 2] >= 130) & (y[:, :, 2] <= 190)
...
def face_color_transfer(target):
    # slab, sb, Sa, Sb, [sab, sae], [sbb, sbe], sam, sbm = deal(source)
    # 随机生成目标源的颜色空间范围,可调
    sab, sae = random.uniform(110,145),random.uniform(110,150)
    sbb, sbe = random.uniform(110,145),random.uniform(110,155)
    ...
    tam = (tab + tae) / 2.0
    tbm = (tbb + tbe) / 2.0
    # print 'sam, sbm, tam, tbm', sam, sbm, tam, tbm
...
def adaptive_color_transform(img_dir, p=0.3):
    if random.random() < p:
        target = imread(img_dir)
        img = face_color_transfer(target)
...
# 输入是图片路径
# 输出是PIL的图片
# 使用时调用 adaptive_color_transform 函数即可
if __name__ == '__main__':
    img_dir = 'images/0001.jpg'
    img = adaptive_color_transform(img_dir, p=0.3)
    img.save('222.jpg')
