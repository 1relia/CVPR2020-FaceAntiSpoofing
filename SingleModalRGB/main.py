...
from utils.data_aug import ColorAugmentation,GetFeature
from utils.spatial_transforms import (
    Compose, Normalize, Scale, CenterCrop, CornerCrop, MultiScaleCornerCrop,
    MultiScaleRandomCrop, RandomHorizontalFlip, ToTensor, Resize)
from utils.crop_eyemouth import EyeMouth
...
parser.add_argument('--mode', default='RGB', type=str, help='Depth, IR, RGB')
parser.add_argument('--save-path', default='checkpoints/feathernetB', type=str, metavar='PATH',
                    help='path to save checkpoint (default: none)')
parser.add_argument('--data-root', default='/home/pengzhang/dataset/CASIA-CeFA/phase1/', type=str, metavar='PATH',
                    help='path to save checkpoint (default: /home/pengzhang/dataset/CASIA-CeFA/phase1/)')
parser.add_argument('--sub-prot-train', default='1', type=str,choices=['1','2','3'],
                    help='sub-protocols (choices: 1,2,3])')
parser.add_argument('--sub-prot-val', default='1', type=str,choices=['1','2','3','12','23','13'],
                    help='sub-protocols (choices: 1,2,3,13,12,23])')
parser.add_argument('--sub-prot-test', default='1', type=str,choices=['1','2','3','12','23','13'],
                    help='sub-protocols (choices: 1,2,3,13,12,23])')
parser.add_argument('--norm-type', default='frn', type=str,choices=['frn','bn','sw2','sw3','sw5','in'],
                    help='normalization type(choices: frn,bn,sw2,sw3,sw5,in)')
...
def main():
    ...
    for k, v in config['common'].items():
        setattr(args, k, v)
    for k, v in config['stage1_model'].items():
        setattr(args, k, v)
    print(args)
    ## Set random seeds ##
...
#########################################################    
    ...
    train_dataset = CASIA(args,
        transforms.Compose([
        # EyeMouth(),
        # MultiScaleRandomCrop([0.6, 1.15], img_size),
        ...
        ColorAugmentation(),
        normalize,
        GetFeature(args,config),
        ]),phase_train=True)
    val_dataset = CASIA(args,
        transforms.Compose([
        # EyeMouth(),
        # Resize(img_size),
        # transforms.Resize(int(224 * 1.)),
        transforms.CenterCrop(img_size),
        transforms.ToTensor(),
        normalize,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_best:
            save_name = '{}/{}_{}_best.pth.tar'.format(args.save_path, args.model_name, epoch)
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
...
#                             dir_ = dirs[i_batch].replace('/home/zp/dataset/CASIA-CeFA/phase1/','').replace(subject_name_list[-1],pre_subject)
                ...
                if i % args.print_freq == 0:
                    ...
                    with open('logs/{}_{}_{}_@{}.log'.format(time_stp, args.arch, args.mode, args.sub_prot_val),'a+') as flog:
                        flog.write('{}\n'.format(line))
                        print(line)
    print(len(label_list), len(predicted_list), confusion_matrix(label_list, predicted_list).ravel())
...
#     pickle.dump(xy_dic, open('xys/xy_{}_{}_{}.pickle'.format(time_stp, args.arch,epoch),'wb'))
    with open('logs/val_result_{}_{}_{}_@{}.txt'.format(time_stp, args.arch, args.mode, args.sub_prot_val),'a+') as f_result:
